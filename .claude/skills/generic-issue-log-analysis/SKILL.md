...
## AutoWSGR Repository Hints

- 任务 API 入口优先看 `autowsgr/server/routes/task.py`，请求模型看 `autowsgr/server/schemas.py`。
- 常规战与活动战执行链路优先看 `autowsgr/ops/normal_fight.py` 与 `autowsgr/ops/event_fight.py`。
- 作战计划解析看 `autowsgr/combat/plan.py`，重点字段：`plan_id`、`fleet_id`、`fleet`。
- 编队切换与按舰船名换船优先看 `autowsgr/ui/battle/fleet_change/` 与 `autowsgr/ui/choose_ship_page.py`。
- 日志关键词可优先检索：`[Task]`、`[OPS]`、`[UI]`、`[Combat]`、`[识别]`。
- 涉及“只切舰队号不换船”的问题时，先核对路由层是否把请求里的 `plan.fleet` 覆盖透传到执行器。
